...
from modules.szz_versions.ag_szz import AGSZZ
from modules.szz_versions.core.abstract_szz import ImpactedFile, DetectLineMoved
...
class MASZZ(AGSZZ):
    ...
    def find_bic(self, fix_commit: str, impacted_files: List['ImpactedFile'], **kwargs) -> Set[Commit]:
        """
        Find bug introducing commits candidates.

        :param str fix_commit: fix commit to scan for buggy commits
        ...
        bic = set()
        for file_id, imp_file in enumerate(impacted_files):
            commits_to_ignore_current_file = commits_to_ignore.copy()

            to_blame = True
            while to_blame:
                log.info(f"excluding commits: {params['ignore_revs_list']}")
                blame_data = self._ag_annotate(fix_commit['hash'], [imp_file], **params)
                ...
                if len(new_commits_to_ignore) == 0 and len(new_commits_to_ignore_current_file) == 0:
                    to_blame = False
                elif ts() - start > (60 * 1):  # 1 minute max time
            ...
            if file_id % 10 == 0:
                log.info(f"Processed {file_id + 1} files out of {len(impacted_files)}")
                print(f"Processed {file_id + 1} files out of {len(impacted_files)}",end='\r')
            bic.update(set([bd.commit for bd in blame_data if bd.commit.hexsha not in commits_to_ignore_current_file]))
