...
from modules.szz_versions.core.comment_parser import parse_comments
...
class AbstractSZZ(ABC):
    ...
    def __init__(self, repo_full_name: str, repo_url: str, repos_dir: str = None, use_temp_dir: bool = True):
        ...
        if not use_temp_dir:
            # TODO: update the temp directory, I updated it
            ...
            if repos_dir:
                ...
                if os.path.isdir(repos_dir):
                    self._repository = Repo(repos_dir)
                else:
                    print('Clone repository', repo_url)
                    Repo.clone_from(url=repo_url, to_path=repos_dir)
                    if os.path.isdir(repos_dir):
                        self._repository = Repo(repos_dir)
                    else:
                        log.error(f'unable to find local repository path: {repos_dir}')
                        exit(-4)
        else:
            ...
            if not os.path.isdir(self._repository_path):
                if repos_dir:
                    ...
                    if os.path.isdir(repos_dir):
                        try:
                            copytree(repos_dir, self._repository_path, symlinks=True)
                        except Exception as e:
                            log.error(f'unable to copy repository: {e}')
                            exit(-4)
                    else:
                        print('Clone repository', repo_url)
                        Repo.clone_from(url=repo_url, to_path=repos_dir)
            ...
            # self._repository.git.fetch('--tags')
            # tags = self._repository.tags
            # self._repository.git.checkout(repository_branch)
    ...
    def get_impacted_files(self, fix_commit: dict,
        ...
        :param str fix_commit: the fix commit to parse
        :returns List[ImpactedFile] impacted_files
        """
        impacted_files = list()
        # repo = PyDrillerGitRepo(self.repository_path)
        # fix_commit = None

        # Traverse the commits and match the hash
        # for commit in repo.traverse_commits():
        #     if commit.hash == fix_commit_hash:
        #         fix_commit = commit
        #         break

        if not fix_commit:
            log.error(f"Commit is not found.")
            return impacted_files

        # Traverse the modified files in the commit
        if fix_commit['has_files']:
            for modification in fix_commit['files']:
                ...
                if file_ext_to_parse:
                    ext = modification['filename'].split('.')[-1]  # Get the extension
                    if ext not in file_ext_to_parse:
                        log.info(f"skip file: {modification['filename']}")
                        continue

                file_path = modification["new_path"] if modification["change_type"] != ModificationType.DELETE else modification["old_path"]

                # Get lines added and deleted
                lines_added = [added[0] for added in modification['added']]
                lines_deleted = [deleted[0] for deleted in modification['deleted']]

                # If only_deleted_lines is True, consider only deleted lines
                if only_deleted_lines:
                    mod_lines = lines_deleted
                else:
                    # Consider lines that are both deleted and added as modified
                    mod_lines = [ld for ld in lines_deleted if ld in lines_added]

                # Append to the impacted files list if there are modified lines
                if len(mod_lines) > 0:
                    impacted_files.append(ImpactedFile(file_path, mod_lines))

            log.info([str(f) for f in impacted_files])

        return impacted_files, fix_commit
